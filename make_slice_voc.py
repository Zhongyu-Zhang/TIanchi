...
def make_slice_voc(outpath,exiset_obj_list,sliceHeight=640, sliceWidth=640):
    name=outpath.split('/')[-1]
    #
    #
    path_ = slice_voc_dir+'/'+name[:-4] + '.xml'
    with codecs.open(path_, 'w', 'utf-8') as xml:
...
###############################################################################
def slice_im(image_path, ann_path,out_name, outdir, sliceHeight=640, sliceWidth=640, zero_frac_thresh=0.2, overlap=0.2, verbose=False):
    #
...
if __name__=="__main__":
    ...
    if use_demo:
        ...
        slice_voc_dir = './yolov5/slice/Annotations'#切出来的标签也保存为voc格式
        outdir = './yolov5/slice/JPEGImages'
